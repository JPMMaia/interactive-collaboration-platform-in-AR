...
namespace CollaborationEngine.RuntimeGizmo
{
    public class TransformGizmo : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            //We run this in lateupdate since coroutines run after update and we want our gizmos to have the updated _target transform position after TransformSelected()
            SetAxisInfo();
            SetLines();
        }
            ...
            while (!Input.GetMouseButtonUp(0))
            {
                ...
                if (previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
                {
                    ...
                    if (type == TransformType.Scale)
                    {
                        ...
                        if (selectedAxis == Axis.Any) _target.localScale += (ExtVector3.Abs(_target.localScale.normalized) * scaleAmount);
                        else _target.localScale += (localAxis * scaleAmount);
        ...
        void SetAxisInfo()
        {
            ...
            axisInfo.Set(_target, size, space);

            if (isTransforming && type == TransformType.Scale)
            {
                if (selectedAxis == Axis.Any) axisInfo.Set(_target, size + totalScaleAmount, space);
        ...
        void SetHandleLines()
        {
            ...
            if (type == TransformType.Move || type == TransformType.Scale)
            {
                handleLines.x.Add(_target.position);
                handleLines.x.Add(axisInfo.xAxisEnd);
                handleLines.y.Add(_target.position);
                handleLines.y.Add(axisInfo.yAxisEnd);
                handleLines.z.Add(_target.position);
        ...
        void SetHandleSquares()
        {
            ...
            if (type == TransformType.Scale)
            {
                ...
                AddSquares(_target.position - (axisInfo.xDirection * boxLength), axisInfo.xDirection, axisInfo.yDirection, axisInfo.zDirection, boxLength, handleSquares.all);
            }
        }
        ...
        void AddCircle(Vector3 origin, Vector3 axisDirection, float size, List<Vector3> resultsBuffer, bool depthTest = true)
        {
            ...
            Plane plane = new Plane((transform.position - _target.position).normalized, _target.position);
